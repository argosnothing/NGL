...
pub fn parse_renderdocs_html(html: &str) -> Result<Vec<ParsedOption>, String> {
    ...
    let dl_selector = Selector::parse("dl.variablelist").map_err(|e| format!("Invalid selector: {:?}", e))?;
    ...
    let term_selector = Selector::parse("span.term").map_err(|e| format!("Invalid selector: {:?}", e))?;
    let option_code_selector = Selector::parse("code.option").map_err(|e| format!("Invalid selector: {:?}", e))?;
    
    let dl = document.select(&dl_selector).next()
    ...
    if dts.len() != dds.len() {
        return Err(format!("Mismatched dt/dd counts: {} dt vs {} dd", dts.len(), dds.len()));
    }
    ...
    for (i, p) in p_elements.iter().enumerate() {
        ...
        for prop_name in ["Type:", "Default:", "Example:"] {
            ...
            let value = if let Some(inline) = extract_inline_value(&p_text, prop_name) {
                ...
                pre_codes.get(i.saturating_sub(p_elements.len() - pre_codes.len()))
...
fn extract_description(dd: &ElementRef) -> Option<String> {
    ...
    for p in dd.select(&p_selector) {
        if p.select(&emphasis_selector).next().is_some() {
            ...
            if text.contains("Type:") || text.contains("Default:") 
                || text.contains("Example:") || text.contains("Declared by:") {
                continue;
            }
        }
